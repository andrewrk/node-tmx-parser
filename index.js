...
function parse(content, pathToFile, cb) {
    ...
    states[STATE_COLLECT_OBJECT_GROUPS] = {
        opentag: function (tag) {
            if (tag.name === 'OBJECT') {
                ...
                object.id = int(tag.attributes.ID);
    ...
    states[STATE_OBJECT] = {
        opentag: function (tag) {
            switch (tag.name) {
                case 'PROPERTIES':
                    if (object.gid && object.type === 'chest') {
                        var obj = object;
                        runRightBeforeCallback.push(function () {
                            var tileSet = topLevelObject.getTileSetByGid(obj.gid);
                            if (tileSet) {
                                var tile = tileSet.tiles[obj.gid - tileSet.firstGid];
                                if (tile && tile.properties) {
                                    obj.properties = Object.assign({}, tile.properties, obj.properties);
                                }
                            }
                        });
                    }
    ...
    parser.onend = function () {
        ...
        pend.wait(function (err) {
            ...
            runRightBeforeCallback.forEach(function (func) {
                func();
            });
...
Map.prototype.getTileSetByGid = function (gid) {
    var self = this;
    return this.tileSets.find(function (ts) {
        var minGid = ts.firstGid;
        var maxGid = ts.firstGid + (ts.image.width / self.tileWidth) * (ts.image.height / self.tileHeight);
